...
module.exports = function (opts) {
  ...
  if (typeof options.realDir !== 'string') { throw Error('realDir must be specified'); }
  ...
  return async (ctx, next) => {
    ...
    if (!options.redirectPath) {
      ...
      if (sent) { return; } else { return next(); }
    }
    if (path.indexOf(options.redirectPath) !== 0) { return next(); }

    if (path === options.redirectPath) { return ctx.redirect(normalize(options.redirectPath + '')); }
    ...
    log && console.log(new Date().toISOString(), path);
    const sent = await send(ctx, path, options);
    if (sent) { return null; } else { return next(); }
  };
};
