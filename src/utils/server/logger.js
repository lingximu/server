...
class Log {
  ...
  _write (entry) {
    ...
    if (entry.priority >= LOG_LEVEL.ERROR) { console.error(log); } else { console.log(log); }
  }
  ...
  _format (entry) {
    ...
    return `[${chalk.gray(moment().format('YYYY-MM-DD HH:mm:ss'))}] ${chalk.bold(LOG_LEVEL_COLOR[entry.priority](entry.priorityLabel))}/${entry.tag || ''} ${entry.identifier ? '<' + entry.identifier + '>' : ''} \n ${entry.message}`;
  }
